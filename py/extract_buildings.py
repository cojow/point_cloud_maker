...
def apply_house_cookie_cutter(ag_pts, ag_colors, footprint_data, ground_model):
    ...
    synthetic floor from the LocalGroundModel. Houses are built on a level
    pad, so rather than terracing the floor to follow every wiggle in the
    ground surface, this samples the ground at every footprint cell and takes
    the median as a single flat elevation for the whole floor - level, but
    still derived from real local ground data instead of one global plane."""
    ...
    if len(grid) > 0:
        ...
        # Level synthetic floor: median true ground elevation across every
        # footprint cell (robust to the odd contour glitch under one corner).
...
def process_reconstruction(args):
    ...
    out = os.path.join(project_path, f"analysis_{project_name}_v1_2")
