...
def process_reconstruction_v21(ply_path):
    output_dir = os.path.join(os.path.dirname(ply_path), "house_analysis_v21")
    ...
    for i in range(labels.max() + 1):
        ...
        if area > 35:
            ...
            for sc_pts in sub_clusters:
                ...
                # --- NEW FIX: The RANSAC + Contiguous Sheet Check ---
                while len(remaining_pts) > 50:
                    pcd_temp = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(remaining_pts))
                    # Stricter distance_threshold (0.15 instead of 0.25)
                    ...
                    # Verify the plane is a solid contiguous roof, not scattered tree points
                    ...
                    if plane_labels.max() >= 0:
                        # Find the largest contiguous chunk of the plane
                        ...
                        # Only keep it if the solid chunk is substantial
                        if len(contiguous_inliers) > 40:
                            normal = np.array(plane_model[:3])
                            normal = normal / np.linalg.norm(normal)
                            if abs(normal[2]) > 0.5: 
                                valid_roof_pts.append(plane_pts[contiguous_inliers])
                    
                    # Remove the original RANSAC inliers so we don't scan them again
                ...
                # Extremely tight Alpha Shape (0.6) to sever thin connecting branches
...
if __name__ == "__main__":
    ...
    process_reconstruction_v21(test_file)